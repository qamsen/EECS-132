...
public class Reversi1 {
  ...
  public Reversi1(int x, int y) {
    ...
    buttonGrid = new JButton[x + 1][y + 1];
  ...
  private boolean isOutOfBounds() {
    ...
    if (getIndexRow() < buttonGrid.length)
      outOfBounds = false;
    if (getIndexColumn() < buttonGrid.length)
      outOfBounds = false;
    if (getIndexRow() > 0)
      outOfBounds = false;
    if (getIndexColumn() > 0)
  ...
  private void gameMove(JButton thisButton) {
    ...
    if(isOpenSquare(thisButton)) {
      if(isLegalDirection(-1, -1)) {
        flipSquares(-1, -1);
        legalMove = true;
      }
      if(isLegalDirection(-1, 0)) {
        flipSquares(-1, 0);
        legalMove = true;
      }
      if(isLegalDirection(-1, 1)) {
